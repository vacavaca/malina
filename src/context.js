...
const defaultContextProvider = state => state.context

export const withContext = (provider = defaultContextProvider) => decorator(Inner => {
  const originalState = state => {
    if (Inner.state instanceof Function) return Inner.state(state)
    else return Inner.state
  }

  const state = state => {
    const context = new Context(provider(state))
    return { ...state, [contextKey]: context }
  }
...
export const getContext = (getter = defaultContextGetter) => decorator(Inner => {
